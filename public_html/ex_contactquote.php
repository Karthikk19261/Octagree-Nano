...
if ($subject === '') $subject = 'Quote request from octagreennano.com';
...
$rows .= og_row('Quote for',     $quotefor);
$rows .= og_row('Approx. area',  $area);
$rows .= og_row('Project notes', $message);

$msg  = "<p style='font-family:sans-serif'>New quote request received via <strong>octagreennano.com</strong></p>";
$msg .= "<table style='background-color:#154360;color:#fff;border-radius:6px;font-family:sans-serif;width:560px;border-collapse:collapse'>{$rows}</table>";
$msg .= "<p style='font-family:sans-serif;color:#666;font-size:12px;margin-top:16px'>Sent from the quote form on octagreennano.com</p>";
...
if ($sent > 0) {
    echo "Your quote request has been received. Our team will get back to you within one business day.";
} else {
